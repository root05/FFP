...
st.set_page_config(page_title="Прогноз на мероприятие",
                   layout="wide",
...
def calculate_guests_and_price(stage1_price,
                               stage1_limit,
                               stage2_price,
                               stage2_limit,
                               stage3_price,
    ...
    for _ in range(max_iterations):
        ...
        estimated_guests = min(total_tickets_available,
                               max(0, round(marketing_guests * price_factor)))

        ticket_sales = {
            'stage1':
            min(estimated_guests, stage1_limit) if estimated_guests > 0 else 0,
            'stage2':
            min(max(0, estimated_guests - stage1_limit), stage2_limit)
            if estimated_guests > stage1_limit else 0,
            'stage3':
            min(max(0, estimated_guests - stage1_limit -
                    stage2_limit), stage3_limit)
            if estimated_guests > stage1_limit + stage2_limit else 0,
            'door':
            min(
                max(
                    0, estimated_guests - stage1_limit - stage2_limit -
                    stage3_limit), door_limit) if door_limit > 0 else max(
                        0, estimated_guests - stage1_limit - stage2_limit -
                        stage3_limit)
        }
...
def main():
    ...
    if 'marketing_values' not in st.session_state:
        st.session_state.marketing_values = {
            name: int(events[name]['marketing_percent'] * 100)
            for name in events.keys()
        }
        st.session_state.marketing_values['New'] = default_marketing
    if 'pre_sale_values' not in st.session_state:
        st.session_state.pre_sale_values = {
            name: default_pre_sale.copy()
            for name in events.keys()
        }
        st.session_state.pre_sale_values['New'] = default_pre_sale.copy()
        st.session_state.pre_sale_values[
            'Hardline I'] = hardline_pre_sale.copy()
        st.session_state.pre_sale_values[
            'Bass Vibration IV'] = bass_vibration_pre_sale.copy()
        st.session_state.pre_sale_values[
            'Neuropunk'] = neuropunk_pre_sale.copy()
    if 'checkbox_states' not in st.session_state:
        st.session_state.checkbox_states = {
            'Neuropunk': True,
            'Bass Vibration IV': True,
            'Hardline I': True,
            'New': True
        }
    if 'free_tickets' not in st.session_state:
        st.session_state.free_tickets = {
            'Neuropunk': 20,
            'Bass Vibration IV': 35,
            'Hardline I': 21,
            'New': 0
        }
    ...
    with col_left:
        ...
        col_settings_left, col_settings_middle, col_settings_right = st.columns(
            [1, 0.25, 1])
        ...
        display_event_name = get_next_version(current_event_name, events,
                                              st.session_state.event_versions)

        if current_event_name != 'New' and display_event_name != st.session_state.event_versions.get(
                current_event_name, current_event_name):
            st.session_state.event_versions[
                current_event_name] = display_event_name

        marketing_percentage = col_settings_left.slider(
            "Маркетинг (%):",
            0,
            100,
            st.session_state.marketing_values.get(current_event_name,
                                                  default_marketing),
            5,
            key=f"marketing_{current_event_name}") / 100
        # Заменил "Проходки:" на "Free:"
        free_tickets = col_settings_middle.number_input(
            "Free:",
            0,
            value=st.session_state.free_tickets.get(current_event_name, 0),
            step=1,
            key=f"free_{current_event_name}")
        new_budget = col_settings_right.number_input(
            "Бюджет (₽):",
            0,
            value=st.session_state.budget_values.get(current_event_name,
                                                     default_budget),
            step=1000,
        ...
        risk_amount = col_settings_right.number_input(
            "Расходы (₽):",
            0,
            value=st.session_state.risk_values.get(current_event_name,
                                                   default_risk),
            step=5000,
        ...
        st.session_state.marketing_values[current_event_name] = int(
            marketing_percentage * 100)
    ...
    with col_right:
        ...
        with col_stage1:
            stage1_price = st.number_input(
                "Цена Этап 1",
                1,
                value=pre_sale['stage1_price'],
                step=100,
                key=f"stage1_price_{current_event_name}")
            stage1_limit = st.number_input(
                "Количество",
                0,
                value=pre_sale['stage1_limit'],
                step=1,
                key=f"stage1_limit_{current_event_name}")
        with col_stage2:
            stage2_price = st.number_input(
                "Цена Этап 2",
                1,
                value=pre_sale['stage2_price'],
                step=100,
                key=f"stage2_price_{current_event_name}")
            stage2_limit = st.number_input(
                "Количество",
                0,
                value=pre_sale['stage2_limit'],
                step=1,
                key=f"stage2_limit_{current_event_name}")
        with col_stage3:
            stage3_price = st.number_input(
                "Цена Этап 3",
                1,
                value=pre_sale['stage3_price'],
                step=100,
                key=f"stage3_price_{current_event_name}")
            stage3_limit = st.number_input(
                "Количество",
                0,
                value=pre_sale['stage3_limit'],
                step=1,
                key=f"stage3_limit_{current_event_name}")
        with col_door:
            door_price = st.number_input(
                "Цена На входе",
                1,
                value=pre_sale['door_price'],
                step=100,
                key=f"door_price_{current_event_name}")
            door_limit = st.number_input(
                "Количество",
                0,
                value=pre_sale['door_limit'],
                step=1,
                key=f"door_limit_{current_event_name}")
        ...
        st.session_state.pre_sale_values[current_event_name] = {
            ...
            'door_limit': door_limit
        }

    marketing_cost = new_budget * marketing_percentage
    min_fame, max_fame = 1.0, 10.15
    min_marketing, max_marketing = 22700, 100000
    fame_factor = min_fame + (max_fame - min_fame) * min(
        1.0,
        max(0.0, (marketing_cost - min_marketing) /
            (max_marketing - min_marketing)))
    ...
    estimated_guests, ticket_sales, avg_ticket_price, ticket_revenue = calculate_guests_and_price(
        stage1_price, stage1_limit, stage2_price, stage2_limit, stage3_price,
        stage3_limit, door_price, door_limit, marketing_guests, free_tickets,
        fame_factor)
    ...
    for event in events.keys():
        if event == current_event_name:
            ...
            df_data['Бюджет'].append(events[event]['budget'] -
                                     events[event]['risk_amount'])
            df_data['Количество гостей'].append(
                events[event]['guests'] +
                st.session_state.free_tickets.get(event, 0))
    ...
    colors = {
        'Neuropunk': 'yellow',
        'Bass Vibration IV': 'green',
        'Hardline I': '#ff005e',
        'New': '#ff00ff'
    }
    visibility = {
        'Neuropunk':
        st.session_state.checkbox_states['Neuropunk'],
        'Bass Vibration IV':
        st.session_state.checkbox_states['Bass Vibration IV'],
        'Hardline I':
        st.session_state.checkbox_states['Hardline I'],
        'New':
        st.session_state.checkbox_states.get('New', True)
        and current_event_name == 'New'
    }
    ...
    for _, row in df.iterrows():
        ...
        if visibility.get(base_event_name, False):
            fig.add_shape(type="line",
                          x0=row['Количество гостей'],
                          y0=0,
                          x1=row['Количество гостей'],
                          y1=row['Стоимость входa'],
                          line=dict(color='#555555', dash="dash", width=1),
                          layer='below')
            fig.add_shape(type="line",
                          x0=0,
                          y0=row['Стоимость входa'],
                          x1=row['Количество гостей'],
                          y1=row['Стоимость входa'],
                          line=dict(color='#555555', dash="dash", width=1),
                          layer='below')
            ...
            fig.add_trace(
                go.Scatter(x=[row['Количество гостей']],
                           y=[row['Стоимость входa']],
                           mode='markers+text',
                           name=event_name,
                           marker=dict(size=15,
                                       color=colors.get(
                                           base_event_name, '#ff00ff'),
                                       opacity=1),
                           text=[event_name],
    ...
    fig.update_layout(xaxis_title="Количество гостей",
    ...
    with col_check1:
        show_neuropunk = st.checkbox(
            "Neuropunk",
            value=st.session_state.checkbox_states['Neuropunk'],
            key="show_neuropunk")
        st.session_state.checkbox_states['Neuropunk'] = show_neuropunk
    with col_check2:
        show_bass_vibration = st.checkbox(
            "Bass Vibration IV",
            value=st.session_state.checkbox_states['Bass Vibration IV'],
            key="show_bass_vibration")
        st.session_state.checkbox_states[
            'Bass Vibration IV'] = show_bass_vibration
    with col_check3:
        show_hardline = st.checkbox(
            "Hardline I",
            value=st.session_state.checkbox_states['Hardline I'],
            key="show_hardline")
        st.session_state.checkbox_states['Hardline I'] = show_hardline
    with col_check4:
        show_new = st.checkbox("New",
                               value=st.session_state.checkbox_states.get(
                                   'New', True),
                               key="show_new")
    ...
    with col_metrics1:
        st.metric("Маркетинг",
                  f"{marketing_cost:,.0f}₽ ({marketing_percentage:.0%})")
    with col_metrics2:
